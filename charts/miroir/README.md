...
| drbd.diskTimeout | int | `600` | disk-timeout in 0.1s units: how long DRBD waits for a backing-device I/O before force-detaching. DRBD's default is 0 (infinite), which wedges a reboot in drbd_md_sync_page_io when the backing disk is failing. Defaulted to 600 (60s). Set to 0 for the DRBD default (not recommended). |
