...
window takes the only UpToDate copy out of service: pods cannot stage
anywhere, the RWX gateway has no healthy replica node to fail over
...
`miroir_volume_resync_ratio` shows progress, and the starter alerts
flag sustained degradation.
...
Requirements section warns about.
...
quorum table (see [At a glance](#at-a-glance)), and the RWX gateway
reschedules onto the surviving replica node (client I/O stalls for
tens of seconds, never errors). Operations that need the missing node
pause and converge when it returns rather than failing:
