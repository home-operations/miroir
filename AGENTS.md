...
- **Idempotent.** Reconcilers, code generators (`mise run generate`), and CLI
  ...
  affects correctness, only latency. In this repo, CSI RPCs (`CreateVolume`,
  `NodePublishVolume`, ...) must be safe to retry: the CSI spec requires it,
  and kubelet/external-provisioner will retry.
...
- **Unit tested**, table-driven via `t.Run` subtests. Stdlib `testing` is the
  ...
  pure functions); reach for `testify` (`assert`/`require`) when a table's
  per-case assertions get repetitive. `sigs.k8s.io/controller-runtime`
  operators keep the kubebuilder scaffold's Ginkgo/Gomega suite rather than
  mixing in a second framework: this repo's controller and CSI suites are
  Ginkgo/Gomega, and the `csi-test/v5` sanity suite (`mise run test-sanity`)
  is the CSI-spec-compliance gate, keep it green. `go test -race` is the
...
- **`log/slog`**, JSON handler to stdout by default (a text-format escape
  ...
  `k8s.io`/`sigs.k8s.io` API requires a `logr.Logger` parameter, don't let
  it leak into this repo's own code as the primary logging interface. Call
...
- **`github.com/spf13/pflag`, only when the app has a real CLI surface**:
  ...
  This repo is the exception in practice: it's a kubebuilder-scaffolded
  controller/CSI driver, so `cmd/main.go` keeps the scaffold's stdlib `flag`
  - `opts.BindFlags(flag.CommandLine)` wiring rather than converting
    generated boilerplate to pflag.
...
`main.go` to wiring: parse config/flags, build the manager, register the
controller and CSI gRPC servers, run, translate the top-level error into an
exit code. Business logic belongs in `internal/controller`,
`internal/<csi-package>`, etc., not in `main`.
...
grep an error message and you know which package raised it. In the CSI gRPC
surface specifically, map internal errors to the correct
`google.golang.org/grpc/codes` (`NotFound`, `AlreadyExists`,
`FailedPrecondition`, ...) at the RPC boundary: callers key retry behavior
off the gRPC status code, not the message text. Define sentinel errors
...
parameter: `Reconcile` and every CSI RPC already get one, propagate it,
don't build a new `context.Background()` partway down the call stack.
Prefer `golang.org/x/sync/errgroup` over raw `sync.WaitGroup` + channels
when fanning out goroutines that can fail: it propagates the first error
and cancels the group's context for you.
...
(deepcopy etc.), `test-integration`, `test-sanity` (CSI spec conformance),
`docs`/`docs-serve`, `helm-*`, and `run-controller`; check `mise tasks`
...
`mise run generate` / `mise run manifests` and check for a diff before
pushing. Lint rules live in `.golangci.yml`; read it instead of trusting a
restated list here, since the two can drift.
...
for a reference implementation); note a CSI node driver typically needs a
privileged/hostPath-mounting DaemonSet regardless of how minimal the image
is, so "static and distroless" only covers the binary, not the Pod's
privilege level. Only drop `CGO_ENABLED=0` if a dependency genuinely
requires cgo. Long-running services in containers should set `GOMEMLIMIT`
via `github.com/KimMachineGun/automemlimit` so the GC reclaims before the
cgroup OOM-kills the process (see `chaski`, `echo`, `konflate`, `ocharted`);
expose Prometheus metrics via `github.com/prometheus/client_golang` on a
separate port from health probes.
